...
// ╚══════════════════════════════════════════════════╝
const INTERNS = [
  {id:1,  name:'Shuja Haider',  username:'shuja',   password:'shuja123'},
  ...
  {id:99, name:'Admin',         username:'admin',   password:'admin123', role:'admin'}
];
...
async function doLogin() {
  ...
  try {
    ...
    err.textContent = '⚠ Cannot connect to server. Check your internet connection.';
...
const adminNav = [
  ...
  {id:'adminNotifs',     icon:'🔔', label:'Task Alerts',   badge:'sb-admin-notif'},
...
function updateBadges() {
  ...
  if (currentRole === 'intern') {
    ...
    if (ela) { ela.textContent = na || ''; ela.className = 'sidebar-badge' + (na ? ' show' : ''); }
    document.getElementById('notifDot').className = 'notif-dot' + ((nc + na) ? ' show' : '');
  }
}
  ...
  ca.innerHTML = `<div>
    ...
    <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:8px;margin-bottom:14px;">
      ${[['🎓','College',s.college||0],['🏢','Office',s.office||0],['🚀','Projects',s.project||0],['🐛','Bugs',s.bugs||0],['💡','Suggestions',s.suggestions||0]]
        .map(([ic,lb,vl]) => `<div class="stat-card"><div class="stat-emoji">${ic}</div><div class="stat-label">${lb}</div><div class="stat-value">${vl}</div></div>`).join('')}
    </div>
...
function refreshContribInfo() {
  ...
  const contrib = db.contributors.find(c => c.category === cat || c.category === 'All');
...
function renderInternTasks(ca) {
  ...
  const all = db.tasks.filter(t => t.assignedTo === id || (t.isPersonal && t.createdBy === id));
  let tasks = all;
  if (filter === 'Today')     tasks = all.filter(t => t.dueDate === td || t.createdDate === td);
  else if (filter === 'Completed') tasks = all.filter(t => isTaskDone(t.id, id));
  else if (filter === 'Pending')   tasks = all.filter(t => !isTaskDone(t.id, id));
...
function renderKanbanCard(task, id) {
  ...
  const due  = task.dueDate ? `<span class="badge badge-orange" style="font-size:10px;">${task.dueDate}</span>` : '';
  const rep  = task.repeat ? `<span style="font-size:10px;color:var(--text3);">🔁</span>` : '';
  let circle;
  if (done)
...
async function completeTask(tid, iid) {
  ...
  if (!task.isPersonal) {
    ...
    db.adminNotifications.unshift({id:'an-'+Date.now(), msg:`${intern?.name||'Intern'} completed: "${task.title}"`, read:false, date:today(), internId:iid, taskId:tid});
  }
...
async function saveCustomTask() {
  ...
  db.tasks.push({
    ...
    createdBy:currentUser.id, assignedTo:currentUser.id, createdDate:today(), points:0, approvalRequired:false
  });
...
async function submitComplaint() {
  ...
  if (!against || !subject) { showToast('Please fill all required fields', 'error'); return; }
  ...
  ca.innerHTML = `<div>
    ...
    <div class="card" style="margin-bottom:12px;">
      ...
      <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:7px;margin-bottom:10px;">
        ${[['🎓','College',s.college||0],['🏢','Office',s.office||0],['🚀','Projects',s.project||0],['🐛','Bugs',s.bugs||0],['💡','Suggestions',s.suggestions||0]]
          .map(([ic,lb,vl]) => `<div class="stat-card"><div class="stat-emoji">${ic}</div><div class="stat-label">${lb}</div><div class="stat-value" style="font-size:20px;">${vl}</div></div>`).join('')}
      </div>
...
function renderAdminTasks(ca) {
  ...
  const fa = window.atFilterAssignee || ''; const fc = window.atFilterCat || '';
  const filtered = allT.filter(t => (!fa || t.assignedTo == fa) && (!fc || (t.category||'').toLowerCase() === fc.toLowerCase()));
  ca.innerHTML = `<div>
...
function setATaskFilter(type, val) { if (type==='a') window.atFilterAssignee=val; else window.atFilterCat=val; navigateTo('adminTasks'); }

function openAdminTaskDetail(tid) {
  const task = db.tasks.find(t => t.id == tid); if (!task) return;
  const intern = INTERNS.find(i => i.id === task.assignedTo); const done = isTaskDone(task.id, task.assignedTo);
  openModal(`<div class="modal-title">${task.title}</div>
...
function openRepeatDlg(ctx) {
  ...
  const ov = document.createElement('div'); ov.id = 'subDialogOverlay';
  ov.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.45);backdrop-filter:blur(2px);z-index:2000;display:flex;align-items:center;justify-content:center;';
  ov._rs = {freq:cur.freq||'Weeks', interval:cur.interval||1, days:[...(cur.days||[])]};
...
function chRI(d) { const ov=document.getElementById('subDialogOverlay'); const el=document.getElementById('riVal'); let v=parseInt(el.textContent)+d; if(v<1)v=1; if(v>999)v=999; el.textContent=v; ov._rs.interval=v; }
function updateRFUI() { const freq=document.getElementById('rfSel').value; const row=document.getElementById('rDaysRow'); if(row) row.style.display=freq==='Weeks'?'flex':'none'; const ov=document.getElementById('subDialogOverlay'); if(ov) ov._rs.freq=freq; }
function togRD(i) { const ov=document.getElementById('subDialogOverlay'); const st=ov._rs; const btn=document.getElementById('rd-'+i); if(st.days.includes(i)){st.days=st.days.filter(d=>d!==i);btn.style.background='#1a1d23';btn.style.color='#aaa';}else{st.days.push(i);btn.style.background='#e85d26';btn.style.color='#fff';} }
function cancelRep() { document.getElementById('subDialogOverlay')?.remove(); }
function confirmRep(ctx) {
  const ov=document.getElementById('subDialogOverlay'); const st=ov._rs;
  _repeatConfig={freq:st.freq, interval:st.interval, days:[...st.days]}; ov.remove();
  const dayNames=['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];
  const label=`Every ${st.interval} ${st.freq}${st.days.length?' ('+st.days.sort().map(d=>dayNames[d]).join(', ')+')':''}`;
  const lel=document.getElementById(ctx==='t'?'tRepLabel':'ptRepLabel'); if(lel) lel.textContent=label;
}
...
async function saveTask() {
  ...
  db.tasks.push({
    id:'t-'+Date.now(), title, description:document.getElementById('tDesc').value,
    assignedTo, category:document.getElementById('tCat').value,
    ...
    isPersonal:false, createdDate:today()
  });
  // Notify the intern
  if (!db.notifications[assignedTo]) db.notifications[assignedTo] = [];
  db.notifications[assignedTo].unshift({id:'n-'+Date.now(), msg:`New task assigned: "${title}"`, type:'task', read:false, date:today()});
...
function renderAdminNotifs(ca) {
  ...
  notifs.forEach(n => n.read = true); saveDB(); updateBadges();
  ca.innerHTML = `<div>
...
async function clearAdminNotifs() { db.adminNotifications = []; await saveAndRefresh('Cleared', 'adminNotifs'); }
...
function renderAdminComplaints(ca) {
  ...
  complaints.forEach(c => c.read = true); saveDB(); updateBadges();
  ca.innerHTML = `<div>
    <div class="page-title">Complaints</div>
...
function openAddContributor() {
  const usedCats  = db.contributors.filter(c => c.category !== 'All').map(c => c.category);
  ...
  if (!allOpt && !catOptions) { showToast('All categories are already assigned', 'error'); return; }
...
async function saveContributor() {
  ...
  db.contributors.push({id:'contrib-'+Date.now(), name, code, category:document.getElementById('cCat').value});
  closeModal();
...
function openEditContrib(id) {
  ...
  const usedCats  = db.contributors.filter(x => x.id !== id && x.category !== 'All').map(x => x.category);
  const availCats = Object.keys(CAT_KEYS).filter(cat => !usedCats.includes(cat));
  const otherHasAll = db.contributors.some(x => x.id !== id && x.category === 'All');
...
async function updateContrib(id) {
  ...
  if (newCode) {
    ...
    db.contributors[idx] = {...db.contributors[idx], name, code:newCode, category:document.getElementById('ecCat').value};
  } else {
    db.contributors[idx] = {...db.contributors[idx], name, category:document.getElementById('ecCat').value};
  }
  closeModal();
