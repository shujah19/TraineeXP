...
// ─── SHA-256 — WebCrypto with pure-JS fallback for Android WebView / file:// ──
// crypto.subtle requires a secure context (HTTPS). When loaded from file:// in
// a WebView it is unavailable, so we fall back to a self-contained JS implementation.
async function sha256(message) {
  ...
  return _sha256js(message);
}

// Compact pure-JS SHA-256 (no dependencies, no external libs)
...
const INTERN_HASHES = {
  ...
  2: '76c4812bf481636d70a49f9c83b16e1cf26a31bbe6948f1be8afd9e697a882c9',
  3: '614b73fd0e83b4bdefa72f698fbe664b13a035f1f05cb27aca348fa740c99f67',
  4: 'df46219531cb5d522d0845901978dccfa286a5b0437f4f9cd4e485064f6b632c',
  ...
  7: 'c63a33856c9d00bf27f3c17c35447bf41f79a824a60ebed45ea3731cc5a9b7dc',
  8: '69dbe507f345251e640dc51d9af47c4421d9b977f059b94327d9e140f360b547',
  9: '54504e4312f9ae005ab40d9d7701adaa20de355a74245929747a9abe4d96d01d',
  10:'655be3101de6f2ba33fa04ca6be960f73e4f10d74f3bc38ebc209cf4a30e01ed',
  11:'cb6b83715df484baed6f54e05e6f461487e962ae1344952e038ced95560648fc',
  12:'a665a45920422f9d417e4867efdc4fb8a04a1f3fff1fa07e998e86f7f7a27ae3',
...
const SETTINGS_HASH = '972bb562f1fffaa9968faca92c5aa8bb98f8cf58d00d2c0c2c1c49457abf0440';

const CAT_KEYS   = {'College Attendance':'college','Office Attendance':'office','Projects':'project','Bugs':'bugs','Suggestions':'suggestions'};
const CAT_ICONS  = {'College Attendance':'🎓','Office Attendance':'🏢','Projects':'🚀','Bugs':'🐛','Suggestions':'💡'};
const DEF_POINTS = {'College Attendance':5,'Office Attendance':5,'Projects':10,'Bugs':8,'Suggestions':5};

// ╔══════════════════════════════════════════════════╗
// ║                    STATE                         ║
// ╚══════════════════════════════════════════════════╝
let currentUser = null, currentRole = 'intern', db = null;
let _approvalContribId = null, _repeatConfig = null;
...
function blankDB() {
  ...
  INTERNS.filter(i => i.id !== 99).forEach(i => {
    d.submissions[i.id]  = {college:0, office:0, project:0, bugs:0, suggestions:0, dailyCounts:{}, points:0};
...
function patchDB(data) {
  ...
  INTERNS.filter(i => i.id !== 99).forEach(i => {
    if (!data.submissions[i.id])
      data.submissions[i.id] = {college:0, office:0, project:0, bugs:0, suggestions:0, dailyCounts:{}, points:0};
...
function processRepeatTasks() {
  ...
  for (const tmpl of templates) {
    ...
    // Pre-populate lastGeneratedByDow from already-existing instances.
    const lastGenByDow = {};
    if (r.freq === 'Weeks') {
      db.tasks
    ...
    for (const checkDate of checkDates) {
      ...
      const alreadyExists = db.tasks.some(t =>
        t.templateId === tmpl.id && t.instanceDate === checkDate
      );
      if (alreadyExists) {
...
function shouldGenerateForDate(r, checkDate, lastGenerated) {
  ...
  if (freq === 'Days') {
    ...
    const daysDiff = Math.floor((new Date(checkDate) - new Date(lastGenerated)) / 86400000);
  ...
  if (freq === 'Weeks') {
    ...
    const weeksDiff = Math.floor((new Date(checkDate) - new Date(lastGenerated)) / (7 * 86400000));
...
function openSettingsLock() {
  ...
  if (!_settingsCooldown) setTimeout(() => document.getElementById('settingsCode')?.focus(), 100);
}
...
async function verifySettingsCode() {
  ...
  if (enteredHash === SETTINGS_HASH) {
    ...
    if (_settingsFails >= 3) {
      ...
      setTimeout(() => { _settingsCooldown = false; _settingsFails = 0; }, 30000);
    }
    openSettingsLock();
  }
}
...
async function doLogin() {
  ...
  if (!user) { showLoginError('Invalid credentials. Please try again.'); return; }
  const enteredHash = await sha256(p);
  if (enteredHash !== INTERN_HASHES[user.id]) { showLoginError('Invalid credentials. Please try again.'); return; }
  ...
  try {
    ...
    // ── PERSIST SESSION: save user id to localStorage so app reopens logged in ──
    try { localStorage.setItem('txp_session', JSON.stringify({ id: user.id })); } catch(e) {}
    document.getElementById('loginScreen').classList.remove('active');
    document.getElementById('mainScreen').classList.add('active');
    document.getElementById('topName').textContent   = sanitize(user.name);
    document.getElementById('topRole').textContent   = currentRole === 'admin' ? 'Administrator' : 'Intern';
    document.getElementById('topAvatar').textContent = user.name[0];
    window.taskFilter = 'Today'; window.adminTaskFilter = 'Sent';
    ...
    hideLoading();
    showLoginError('⚠ Cannot connect to server. Check your internet connection.');
  }
}
...
function doLogout() {
  if (_repeatCheckTimer) { clearInterval(_repeatCheckTimer); _repeatCheckTimer = null; }
  // ── CLEAR SESSION: only wipe localStorage when user explicitly logs out ──
  try { localStorage.removeItem('txp_session'); } catch(e) {}
  currentUser = null; currentRole = 'intern'; db = null; currentPage = null;
  settingsUnlocked = false; _settingsFails = 0; _settingsCooldown = false;
...
function toggleMobileSidebar() {
  ...
  if (open) { sb.classList.remove('mobile-open'); ov.classList.remove('open'); }
...
function updateBadges() {
  ...
  if (currentRole === 'intern') {
    ...
    document.getElementById('notifDot').className = 'notif-dot' + (n ? ' show' : '');
  } else {
    ...
    document.getElementById('notifDot').className = 'notif-dot' + ((nc + na + nr) ? ' show' : '');
  }
}
...
function getObedience(id) {
  ...
  const cutoffStr = dateToLocalStr(new Date(Date.now() - 30 * 24 * 3600 * 1000));
  const tasks = db.tasks.filter(t =>
    t.assignedTo === id && !t.isPersonal && !t.isTemplate &&
    (!t.instanceDate || t.instanceDate >= cutoffStr)
  );
  if (!tasks.length) return 0;
  return Math.round(tasks.filter(t => db.taskCompletions?.[t.id + '-' + id]?.done).length / tasks.length * 100);
}
...
function getObedienceCounts(id) {
  ...
  return { total: tasks.length, done: tasks.filter(t => db.taskCompletions?.[t.id + '-' + id]?.done).length };
}


// STREAK: consecutive calendar days on which the intern completed ≥1 task.
// Looks at all completed tasks (non-personal, non-template) and counts
// the longest run of consecutive days ending today or most recent active day.
function getStreak(id) {
  // Collect all dates on which this intern completed a task
  const completedDates = new Set();
  db.tasks.forEach(t => {
    if (t.assignedTo !== id || t.isPersonal || t.isTemplate) return;
    if (!db.taskCompletions?.[t.id + '-' + id]?.done) return;
    // Use instanceDate if present, else createdDate, else skip
    const d = t.instanceDate || t.dueDate || t.createdDate;
  ...
  // Count longest streak of consecutive days
...
function renderInternDashboard(ca) {
  const id = currentUser.id; const s = db.submissions[id] || {};
  // BUG-24 FIX: use toLocaleString() for thousands separators (1,250 not 1250)
  const pts = s.points || 0; const ob = getObedience(id);
...
function refreshContribInfo() {
  const cat = document.getElementById('addCat')?.value; if (!cat) return;
  const catPts = db.catPoints || DEF_POINTS;
  ...
  const contrib = db.contributors.find(c => !c.taskApprovalOnly && (c.category === cat || c.category === 'All'));
...
async function submitAdd() {
  ...
  if (contrib.category && contrib.category !== cat && contrib.category !== 'All') {
    showToast('This contributor is not assigned to ' + cat, 'error'); return;
  }
...
function openSendRequest(preTaskId) {
  ...
  const alreadyRequested = new Set(
    (db.pendingRequests || []).filter(r => r.status === 'pending' && r.type === 'task').map(r => r.taskId)
  );
  const approvalTasks = db.tasks.filter(t =>
    t.assignedTo === currentUser.id && !t.isPersonal && !t.isTemplate &&
    t.approvalRequired === true && !isTaskDone(t.id, currentUser.id) && !alreadyRequested.has(t.id)
  );
...
function toggleReqType() {
  ...
  document.getElementById('reqEntrySection').style.display = type === 'entry' ? 'block' : 'none';
  document.getElementById('reqTaskSection').style.display  = type === 'task'  ? 'block' : 'none';
}

async function submitRequest() {
  const type = document.getElementById('reqType').value;
  const note = document.getElementById('reqNote')?.value.trim() || '';
  let reqData = { id: 'req-'+Date.now(), type, internId: currentUser.id, note, date: today(), status: 'pending', read: false };
  if (type === 'entry') {
    ...
    if (!taskId) { showToast('No approval-required tasks available', 'error'); return; }
    // Guard: no duplicate pending request for same task
    const alreadyPending = (db.pendingRequests || []).some(r => r.taskId === taskId && r.status === 'pending');
    if (alreadyPending) { showToast('A request for this task is already pending', 'error'); return; }
...
async function approveRequest(reqId) {
  ...
  if (req.type === 'entry') {
    ...
    if (!db.taskCompletions[key]?.done) {
      ...
      if (task?.points) { s.points = (s.points || 0) + task.points; db.submissions[iid] = s; }
      if (!db.notifications[iid]) db.notifications[iid] = [];
...
// ║              INTERN TASKS (KANBAN)               ║
// ╚══════════════════════════════════════════════════╝
function isTaskDone(tid, iid) { return db.taskCompletions?.[tid + '-' + iid]?.done || false; }

// Sort tasks newest-first
function sortNewest(tasks) {
  return [...tasks].sort((a, b) => {
    const ta = a.createdAt || (a.createdDate ? new Date(a.createdDate).getTime() : 0);
    const tb = b.createdAt || (b.createdDate ? new Date(b.createdDate).getTime() : 0);
    return tb - ta;
  });
}

function renderInternTasks(ca) {
  const id = currentUser.id; const filter = window.taskFilter || 'Today'; const td = today();
  let all = db.tasks.filter(t => (t.assignedTo === id || (t.isPersonal && t.createdBy === id)) && !t.isTemplate);
  all = sortNewest(all);
  let tasks = all;
  if (filter === 'Today')
    tasks = all.filter(t => t.dueDate === td || t.createdDate === td || t.instanceDate === td);
  ...
  const ribbon = unread.length ? (() => {
    const taskNotifs = unread.filter(n => n.type === 'task' || n.type === 'repeat');
    const otherNotifs = unread.filter(n => n.type !== 'task' && n.type !== 'repeat');
  ...
  const totalTasks = db.tasks.filter(t => (t.assignedTo === id || (t.isPersonal && t.createdBy === id)) && !t.isTemplate).length;
  const emptyAllState = totalTasks === 0 ? `
  ...
  ca.innerHTML = `<div>
    ...
    <div class="task-filter-bar">
      ${['Today','All','Completed','Pending'].map(f =>
        `<button class="task-filter-btn ${filter === f ? 'active' : ''}" onclick="setTF('${f}')">${f}</button>`).join('')}
...
function renderKanbanCol(cat, tasks, id) {
  ...
  const colTasks = tasks.filter(t => (t.category || 'Personal').toLowerCase() === cls);
...
async function deletePersonalTask(tid) {
  ...
  if (!task || !task.isPersonal || task.createdBy != currentUser.id) {
    showToast('You can only delete your own tasks', 'error'); return;
  }
...
function openTaskDetail(tid) {
  const task = db.tasks.find(t => t.id == tid); if (!task) return;
  // BUG-12 FIX: interns can only view their own tasks
  if (currentRole === 'intern') {
    const isOwned = task.assignedTo === currentUser.id || (task.isPersonal && task.createdBy == currentUser.id);
    if (!isOwned) { showToast('Access denied', 'error'); return; }
  }
  ...
  openModal(`<div class="modal-title">${sanitize(task.title)}</div>
    <div style="background:var(--surface2);border-radius:8px;padding:12px;font-size:13px;color:var(--text2);line-height:1.7;margin-bottom:12px;">${sanitize(task.description) || '<em>No description.</em>'}</div>
...
async function completeTask(tid, iid) {
  ...
  showUndoToast(`Task completed! +${pts} pts`, () => {
    // ─── UNDO ───
    delete db.taskCompletions[key];
    if (pts) db.submissions[iid].points = Math.max(0, (db.submissions[iid].points || 0) - pts);
    renderInternTasks(document.getElementById('contentArea'));
    updateBadges();
  }, async () => {
    // ─── COMMIT ───
...
function openApprovalDlg(tid, iid) {
  const task = db.tasks.find(t => t.id == tid);
  const ac = task.approvalContribId ? db.contributors.find(c => c.id === task.approvalContribId) : null;
...
async function submitApproval(tid, iid) {
  ...
  if (task.approvalContribId && contrib.id !== task.approvalContribId) {
    ...
    showToast('Wrong contributor. Required: ' + (ac?.name || 'assigned contributor'), 'error'); return;
  }
  const key = tid + '-' + iid;
  db.taskCompletions[key] = {done: true};
...
async function saveCustomTask() {
  ...
  db.tasks.unshift({
    id:'pt-'+Date.now(), title, description:document.getElementById('ptDesc').value,
    dueDate:document.getElementById('ptDue').value, repeat:_repeatConfig,
    category:document.getElementById('ptCat').value, isPersonal:true,
    createdBy:currentUser.id, assignedTo:currentUser.id,
    createdDate:today(), createdAt:Date.now(),
    points:0, approvalRequired:false,
    isTemplate: hasRepeat, lastGenerated: hasRepeat ? null : undefined
  });
  closeModal(); _repeatConfig = null;
...
async function submitComplaint() {
  ...
  if (!against) { showToast('Please select an intern to file against', 'error'); return; }
  if (!subject) { showToast('Subject is required', 'error'); return; }
  db.complaints.push({id:'c-'+Date.now(), from:currentUser.id, against:parseInt(against), subject, desc, date:today(), read:false});
  closeModal();
...
function renderAdminDashboard(ca) {
  ...
  const sortKey = window.adminSort || 'pts';
  const sortDir = window.adminSortDir !== false; // true = desc

  // Build per-intern data with streak
  const internData = interns.map(i => {
    ...
    const streak = getStreak(i.id);
    return { intern: i, pts: s.points || 0, ob, total, done, s, streak };
  });

  // Sort by selected key
  internData.sort((a, b) => {
    if (sortKey === 'name') {
      return sortDir ? b.intern.name.localeCompare(a.intern.name) : a.intern.name.localeCompare(b.intern.name);
    }
  ...
  const topPerformer = [...internData].sort((a,b) => b.pts - a.pts)[0];

  // Highest streak stat
  const highestStreakData = [...internData].sort((a,b) => b.streak - a.streak)[0];
  const highestStreakName = highestStreakData ? highestStreakData.intern.name : '—';
  const highestStreakVal  = highestStreakData ? highestStreakData.streak : 0;
  ...
  // Build table rows — rank always reflects position-in-sorted-list
  const tableRows = internData.map((d, idx) => {
    const rank = idx + 1;
    // Clear rank display: medal emoji for top 3, plain bold number for rest
    const rankCell = rank === 1
      ? `<span style="font-size:18px;" title="Rank 1">🥇</span>`
      : rank === 2
      ? `<span style="font-size:18px;" title="Rank 2">🥈</span>`
      : rank === 3
      ? `<span style="font-size:18px;" title="Rank 3">🥉</span>`
      : `<span style="font-size:13px;font-weight:700;color:var(--text2);">${rank}</span>`;

    const obColor = d.ob >= 70 ? 'var(--accent3)' : d.ob >= 40 ? 'var(--yellow)' : '#e53e3e';
    const streakColor = d.streak >= 7 ? 'var(--accent)' : d.streak >= 3 ? 'var(--yellow)' : 'var(--text2)';
    const streakIcon  = d.streak >= 7 ? '🔥' : d.streak >= 3 ? '⚡' : '—';

    return `<tr class="admin-overview-row" onclick="openInternDetailModal(${d.intern.id})" style="cursor:pointer;" title="Click to view ${sanitize(d.intern.name)}'s full details">
      <td style="text-align:center;width:44px;">${rankCell}</td>
      <td>
        <div style="display:flex;align-items:center;gap:8px;">
          <div style="width:30px;height:30px;border-radius:50%;background:var(--accent);display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:800;color:#fff;flex-shrink:0;">${sanitize(d.intern.name[0])}</div>
          <div>
      ...
      <td style="text-align:center;">
        <span style="font-size:13px;font-weight:700;color:${streakColor};">${streakIcon} ${d.streak > 0 ? d.streak + 'd' : '0d'}</span>
      </td>
    </tr>`;
  }).join('');
  ...
  ca.innerHTML = `<div>
    ...
    <div class="page-sub">All-intern performance overview — click any row to view details</div>
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:18px;">
      ...
      <div class="stat-card">
        ...
        <div style="font-size:13px;font-weight:700;margin-top:4px;">${topPerformer ? sanitize(topPerformer.intern.name) : '—'}</div>
        <div style="font-size:11px;color:var(--text2);">${topPerformer ? topPerformer.pts.toLocaleString()+' pts' : ''}</div>
      </div>
      <div class="stat-card" style="border-color:rgba(232,93,38,.3);background:rgba(232,93,38,.04);">
        <div class="stat-label" style="color:var(--accent);">🔥 Highest Streak</div>
        <div style="font-size:16px;font-weight:800;color:var(--accent);margin-top:4px;">${sanitize(highestStreakName)}</div>
        <div style="font-size:11px;color:var(--text2);">${highestStreakVal} day${highestStreakVal !== 1 ? 's' : ''} streak</div>
      </div>
    </div>
    ...
    <div class="card" style="padding:0;overflow:hidden;margin-bottom:18px;">
      <div style="padding:12px 16px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;">
        ...
        <div style="font-size:11px;color:var(--text2);">Click a row to view full details · Click header to sort</div>
      </div>
      <div style="overflow-x:auto;">
        <table class="table" style="width:100%;">
          <thead>
            <tr>
              <th style="text-align:center;width:44px;">RANK</th>
              ${sortTh('NAME','name')}
              ${sortTh('POINTS','pts')}
              ${sortTh('OBEDIENCE','ob')}
              <th style="text-align:center;">TASKS DONE</th>
              ${sortTh('🔥 STREAK','streak')}
            </tr>
          </thead>
...
function renderRecurringDashboard() {
  ...
  const rows = templates.map(tmpl => {
    ...
    const done    = insts.filter(t => isTaskDone(t.id, tmpl.assignedTo)).length;
    const rate    = insts.length ? Math.round(done / insts.length * 100) : 0;
    const barColor = rate >= 75 ? '#27ae60' : rate >= 40 ? '#f59e0b' : '#e53e3e';
    const pausedHtml = tmpl.paused ? '<span class="badge badge-red" style="font-size:9px;margin-left:4px;">⏸ Paused</span>' : '';
...
function renderAdminTaskOverview(sid) {
  ...
  let tasks = db.tasks.filter(t => t.assignedTo === sid && !t.isPersonal && !t.isTemplate);
  tasks = sortNewest(tasks);
  const filtered = filter === 'Completed' ? tasks.filter(t => isTaskDone(t.id, sid)) : tasks.filter(t => !isTaskDone(t.id, sid));
  return `<div style="display:flex;gap:5px;margin-bottom:9px;">
    ${['Sent','Completed'].map(f =>
      `<button class="task-filter-btn ${(window.adminTaskFilter||'Sent')===f?'active':''}" onclick="setATF('${f}')">${f}</button>`).join('')}
  </div>
  ${filtered.length === 0 ? `<div style="color:var(--text2);text-align:center;padding:18px;">No tasks.</div>` :
    `<div style="display:flex;flex-direction:column;gap:5px;">${filtered.map(t => {
...
function openInternDetailModal(internId) {
  ...
  const { total, done } = getObedienceCounts(internId);
  const streak = getStreak(internId);
  const obColor = ob >= 70 ? '#27ae60' : ob >= 40 ? '#f59e0b' : '#e53e3e';
  const streakColor = streak >= 7 ? 'var(--accent)' : streak >= 3 ? '#f59e0b' : 'var(--text2)';

  // Task overview for this intern
  const allTasks = db.tasks.filter(t => t.assignedTo === internId && !t.isPersonal && !t.isTemplate);
  const pendingTasks   = allTasks.filter(t => !isTaskDone(t.id, internId));
  const completedTasks = allTasks.filter(t =>  isTaskDone(t.id, internId));

  function taskCard(t, isDone) {
    const catColor = {'College Attendance':'#6366f1','Office Attendance':'#0ea5e9','Projects':'#f59e0b','Bugs':'#e53e3e','Suggestions':'#27ae60'}[t.category] || '#9ca3af';
    return `<div style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:var(--surface2);border-radius:8px;border-left:3px solid ${catColor};">
      <div style="width:20px;height:20px;border-radius:50%;border:2px solid ${isDone?'#27ae60':'var(--border)'};background:${isDone?'#27ae60':'transparent'};display:flex;align-items:center;justify-content:center;flex-shrink:0;">
        ${isDone?'<i data-lucide="check" style="width:10px;height:10px;color:#fff;"></i>':''}
      </div>
      <div style="flex:1;min-width:0;">
        <div style="font-size:13px;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;${isDone?'text-decoration:line-through;opacity:.55;':''}">${sanitize(t.title)}</div>
        <div style="font-size:11px;color:var(--text2);margin-top:1px;">${t.category||''}${t.points?` · ⭐${t.points} pts`:''}</div>
      </div>
    </div>`;
  }

  const pendingHtml   = pendingTasks.slice(0,8).map(t => taskCard(t,false)).join('');
  const completedHtml = completedTasks.slice(0,8).map(t => taskCard(t,true)).join('');

  const catStats = [
    ['graduation-cap','COLLEGE',  s.college||0],
    ['building-2',    'OFFICE',   s.office||0],
    ['rocket',        'PROJECTS', s.project||0],
    ['bug',           'BUGS',     s.bugs||0],
    ['lightbulb',     'SUGGESTIONS', s.suggestions||0],
  ];
  ...
  openModal(`
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px;gap:12px;flex-wrap:wrap;">
      <div style="display:flex;align-items:center;gap:12px;">
        <div style="width:52px;height:52px;border-radius:50%;background:var(--accent);display:flex;align-items:center;justify-content:center;font-size:22px;font-weight:800;color:#fff;flex-shrink:0;">${sanitize(intern.name[0])}</div>
        <div>
          <div style="font-size:18px;font-weight:800;font-family:'Outfit',sans-serif;">${sanitize(intern.name)}</div>
          <div style="font-size:12px;color:var(--text2);">@${sanitize(intern.username)}</div>
        </div>
      </div>
      <div style="text-align:right;">
        <div style="font-size:11px;color:var(--text2);font-weight:600;text-transform:uppercase;letter-spacing:.5px;">Total Points</div>
        <div style="font-size:32px;font-weight:800;color:var(--accent);font-family:'Outfit',sans-serif;line-height:1;">${pts.toLocaleString()}</div>
      </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:16px;">
      <div style="background:var(--surface2);border-radius:10px;padding:12px;text-align:center;">
        <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:var(--text2);margin-bottom:4px;">Obedience</div>
        <div style="font-size:22px;font-weight:800;color:${obColor};font-family:'Outfit',sans-serif;">${ob}%</div>
        <div style="height:4px;background:var(--border);border-radius:2px;overflow:hidden;margin-top:6px;">
          <div style="height:100%;width:${ob}%;background:${obColor};border-radius:2px;"></div>
        </div>
        <div style="font-size:10px;color:var(--text3);margin-top:4px;">${done}/${total} tasks</div>
      </div>
      <div style="background:var(--surface2);border-radius:10px;padding:12px;text-align:center;">
        <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:var(--text2);margin-bottom:4px;">🔥 Streak</div>
        <div style="font-size:22px;font-weight:800;color:${streakColor};font-family:'Outfit',sans-serif;">${streak}d</div>
        <div style="font-size:10px;color:var(--text3);margin-top:10px;">${streak >= 7 ? 'On fire! 🔥' : streak >= 3 ? 'Building up ⚡' : 'Keep going!'}</div>
      </div>
      <div style="background:var(--surface2);border-radius:10px;padding:12px;text-align:center;">
        <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:var(--text2);margin-bottom:4px;">Tasks Done</div>
        <div style="font-size:22px;font-weight:800;font-family:'Outfit',sans-serif;">${done}</div>
        <div style="font-size:10px;color:var(--text3);margin-top:10px;">${total} assigned</div>
      </div>
    </div>

    <!-- Category breakdown -->
    <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:var(--text2);margin-bottom:8px;">Performance Breakdown</div>
    <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:6px;margin-bottom:16px;">
      ${catStats.map(([ic,lb,vl]) => `
        <div style="background:var(--surface2);border-radius:8px;padding:10px 6px;text-align:center;">
          <div style="margin-bottom:5px;display:flex;justify-content:center;"><i data-lucide="${ic}" style="width:16px;height:16px;color:var(--accent);"></i></div>
          <div style="font-size:9px;font-weight:700;color:var(--text2);text-transform:uppercase;letter-spacing:.4px;margin-bottom:3px;">${lb}</div>
          <div style="font-size:18px;font-weight:800;font-family:'Outfit',sans-serif;">${vl.toLocaleString()}</div>
        </div>`).join('')}
    </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;max-height:260px;overflow-y:auto;">
      <div>
        <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:var(--text2);margin-bottom:6px;">Pending (${pendingTasks.length})</div>
        <div style="display:flex;flex-direction:column;gap:5px;">
          ${pendingHtml || '<div style="color:var(--text3);font-size:12px;padding:8px;">None pending</div>'}
        </div>
      </div>
      <div>
        <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:#27ae60;margin-bottom:6px;">Completed (${completedTasks.length})</div>
        <div style="display:flex;flex-direction:column;gap:5px;">
          ${completedHtml || '<div style="color:var(--text3);font-size:12px;padding:8px;">None yet</div>'}
        </div>
      </div>
    </div>` : '<div style="color:var(--text2);text-align:center;padding:16px;font-size:13px;">No tasks assigned yet.</div>'}
...
function setATF(f) { window.adminTaskFilter = f; }
...
function renderAdminTasks(ca) {
  ...
  let filtered = allT.filter(t => (!fa || t.assignedTo == fa) && (!fc || (t.category||'').toLowerCase() === fc.toLowerCase()));
  filtered = sortNewest(filtered);
  if (fs === 'Pending')      filtered = filtered.filter(t => !t.isTemplate && !isTaskDone(t.id, t.assignedTo));
  if (fs === 'Completed')    filtered = filtered.filter(t => isTaskDone(t.id, t.assignedTo));
    ...
    <div class="card table-wrap" style="padding:0;overflow-x:auto;">
      <table class="table">
        ...
        <tbody>
          ...
          ${filtered.map(t => {
            const intern = INTERNS.find(i => i.id === t.assignedTo); const done = isTaskDone(t.id, t.assignedTo);
            const instCount  = t.isTemplate ? db.tasks.filter(x => x.templateId === t.id).length : 0;
            const pausedBadge = t.isTemplate && t.paused ? `<span class="badge badge-red" style="font-size:9px;">⏸ Paused</span>` : '';
...
function openAdminTaskDetail(tid) {
  ...
  if (task.isTemplate) {
    ...
    const totalDone = instances.filter(t => isTaskDone(t.id, task.assignedTo)).length;
    const compRate  = instances.length ? Math.round(totalDone / instances.length * 100) : 0;
...
async function togglePauseTemplate(tid) {
  ...
  closeModal();
  await saveAndRefresh(task.paused ? '⏸ Recurring task paused' : '▶ Recurring task resumed', 'adminTasks');
}
...
function openEditTask(tid) {
  ...
  const interns = INTERNS.filter(i => i.id !== 99);
  const contribName = task.approvalContribId ? (db.contributors.find(c=>c.id===task.approvalContribId)?.name||'Unknown') : '';
  ...
  openModal(`<div class="modal-title">✏️ Edit Task</div>
    ...
    ${task.isTemplate ? `<div style="background:#fffbeb;border:1px solid #fde68a;border-radius:8px;padding:9px 13px;font-size:12px;color:#92400e;margin-bottom:10px;">
...
function filterEditInternPicker() {
  const q = (document.getElementById('etInternSearch')?.value || '').toLowerCase();
...
async function saveEditTask(tid) {
  ...
  const radioChecked = document.querySelector('input[name="etAssignRadio"]:checked');
  const newAssignedTo = radioChecked ? parseInt(radioChecked.value) : oldAssignedTo;
  task.title       = title;
...
async function doDeleteAdminTask(tid) {
  ...
  db.tasks.filter(t => t.templateId === tid).forEach(inst => {
  ...
  Object.keys(db.taskCompletions).forEach(k => { if (k.startsWith(tid + '-')) delete db.taskCompletions[k]; });
  // Also clean up any pending requests referencing this task
  if (db.pendingRequests) db.pendingRequests = db.pendingRequests.filter(r => r.taskId !== tid);
  closeModal();
...
function toggleAllInterns(check) {
  document.querySelectorAll('#tAssignList label:not(.pick-all-row)').forEach(lbl => {
    if (lbl.style.display !== 'none') lbl.querySelector('input').checked = check;
  });
  updateAssignCount();
}
...
function filterInternPicker() {
  const q = (document.getElementById('tInternSearch')?.value || '').toLowerCase();
...
function handleApprovalToggle(cb) {
  ...
  else { _approvalContribId = null; document.getElementById('approvalContribDisplay').style.display = 'none'; }
}

function openApprovalContribDlg() {
  document.getElementById('subDialogOverlay')?.remove();
  const ov = document.createElement('div'); ov.id = 'subDialogOverlay';
  ov.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.45);backdrop-filter:blur(2px);z-index:2000;display:flex;align-items:center;justify-content:center;';
...
function cancelApprovalDlg() {
  document.getElementById('subDialogOverlay')?.remove();
  const t = document.getElementById('tApproval'); if (t) t.checked = false;
  _approvalContribId = null;
  const d = document.getElementById('approvalContribDisplay'); if (d) d.style.display = 'none';
}

function confirmApprovalDlg() {
  const sel = document.getElementById('subContribSel');
  if (!sel) { document.getElementById('subDialogOverlay')?.remove(); return; }
  const contrib = db.contributors.find(c => c.id === sel.value);
  _approvalContribId = sel.value;
  document.getElementById('subDialogOverlay')?.remove();
  const d = document.getElementById('approvalContribDisplay'); const l = document.getElementById('approvalContribLabel');
  if (d && l && contrib) { l.textContent = '✓ Contributor: ' + contrib.name; d.style.display = 'block'; }
}
...
function repeatLabel(r) {
  ...
  if (r.freq === 'Weeks') {
    const dayStr = r.days?.length
      ? ' (' + r.days.slice().sort((a,b)=>a-b).map(d=>dn[d]).join(', ') + ')'
      : '';
    return r.interval === 1 ? `Every Week${dayStr}` : `Every ${r.interval} Weeks${dayStr}`;
  }
  if (r.freq === 'Months')  return r.interval === 1 ? 'Monthly'   : `Every ${r.interval} Months`;
  if (r.freq === 'Years')   return r.interval === 1 ? 'Yearly'    : `Every ${r.interval} Years`;
...
function startRepeatTimer() {
  ...
  _repeatCheckTimer = setInterval(async () => {
    if (!currentUser || !db) return;
    const changed = processRepeatTasks();
    if (changed) {
      try {
      ...
      } catch(e) { /* silent — will catch on next sync */ }
    }
...
function openRepeatDlg(ctx) {
  ...
  const showDays   = (initPreset === 'weekly') || (initPreset === 'custom' && cur.freq === 'Weeks');
  const showCustom = initPreset === 'custom';

  const ov = document.createElement('div');
  ov.id = 'subDialogOverlay';
  ov.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.45);backdrop-filter:blur(2px);z-index:2000;display:flex;align-items:center;justify-content:center;';
  ov._rs = {freq: cur.freq||'Days', interval: cur.interval||1, days:[...(cur.days||[])], preset: initPreset};

  const pillStyle  = (active) => `padding:6px 14px;border-radius:20px;border:1.5px solid ${active?'#e85d26':'#444'};background:${active?'#e85d26':'transparent'};color:${active?'#fff':'#a8adc0'};font-size:12px;font-weight:600;cursor:pointer;font-family:inherit;transition:all .15s;`;
  const presets = [{k:'daily','label':'⚡ Daily'},{k:'weekdays',label:'📅 Weekdays'},{k:'weekly',label:'🗓 Weekly'},{k:'monthly',label:'📆 Monthly'},{k:'custom',label:'⚙ Custom'}];
...
function selectRepPreset(p) {
  const ov = document.getElementById('subDialogOverlay'); if (!ov) return;
  ov._rs.preset = p;
  // Update pill styles
  ['daily','weekdays','weekly','monthly','custom'].forEach(k => {
    const btn = document.getElementById('rp-'+k); if (!btn) return;
    const active = k === p;
  ...
  // Apply preset config
  const map = {daily:{freq:'Days',interval:1}, weekdays:{freq:'Weekdays',interval:1},
               weekly:{freq:'Weeks',interval:1}, monthly:{freq:'Months',interval:1}};
  if (map[p]) { ov._rs.freq = map[p].freq; ov._rs.interval = map[p].interval; }
  ...
  const rfSel = document.getElementById('rfSel'); if (rfSel) rfSel.value = ov._rs.freq;
  const riVal = document.getElementById('riVal'); if (riVal) riVal.textContent = ov._rs.interval;
  // Show/hide day picker
  const showDays = p==='weekly' || (p==='custom' && ov._rs.freq==='Weeks');
  const dr = document.getElementById('rDaysRow'); if (dr) dr.style.display = showDays?'flex':'none';
  updateRepPreview();
}
...
function updateRepPreview() {
  const ov  = document.getElementById('subDialogOverlay'); if (!ov) return;
  const lbl = document.getElementById('repPreviewLabel'); if (!lbl) return;
...
function chRI(d) {
  ...
  el.textContent = v; ov._rs.interval = v; updateRepPreview();
}
...
function togRD(i) {
  ...
  if (st.days.includes(i)) {
    ...
    btn.style.background='#1a1d23'; btn.style.color='#a8adc0';
  } else {
    st.days.push(i);
    btn.style.background='#e85d26'; btn.style.color='#fff';
  }
  updateRepPreview();
}

function cancelRep() { document.getElementById('subDialogOverlay')?.remove(); }
...
function confirmRep(ctx) {
  ...
  if ((st.preset==='weekly' || st.freq==='Weeks') && st.days.length===0) {
    document.getElementById('repPreviewLabel').textContent = '⚠ Select at least one day';
    document.getElementById('repPreviewLabel').style.color = '#e53e3e'; return;
  }
...
async function saveTask() {
  ...
  if (approvalRequired && !_approvalContribId) { showToast('Select a contributor for approval', 'error'); openApprovalContribDlg(); return; }
  ...
  closeModal(); _approvalContribId = null; _repeatConfig = null;
  ...
  if (hasRepeat) processRepeatTasks();
  await saveAndRefresh(`Task created for ${selectedIds.length} intern(s)!`, 'adminTasks');
}
...
function renderAdminNotifs(ca) {
  ...
  // BUG-20 FIX: only write to Firebase if there were actually unread notifications to mark
...
async function markComplaintDone(id) {
  const c = db.complaints.find(x => x.id === id); if (!c) return;
  c.done = true;
...
function renderAdminSettings(ca) {
  const contribs = db.contributors || []; const catPts = db.catPoints || DEF_POINTS;
  ca.innerHTML = `<div>
    ...
    <div class="card" style="margin-bottom:16px;">
      ...
      ${Object.entries(catPts).map(([cat,pts]) =>
        `<div class="point-input-row"><span class="point-input-label">${CAT_ICONS[cat]||''} ${cat}</span><input type="number" class="point-input-field" id="cp-${cat.replace(/\s+/g,'_')}" value="${pts}" min="0"></div>`).join('')}
      ...
      <div class="card">
        ...
        <div class="form-group" style="margin-bottom:9px;">
          ...
          <select id="exportInternSel" style="border:1.5px solid var(--border);border-radius:8px;padding:7px;font-size:13px;font-family:inherit;width:100%;outline:none;background:#fff;">
            ${INTERNS.filter(i => i.id !== 99).map(i => `<option value="${i.id}">${i.name}</option>`).join('')}
          </select>
        </div>
...
function openAddContributor() {
  const usedCats  = db.contributors.filter(c => c.category !== 'All' && !c.taskApprovalOnly).map(c => c.category);
  const availCats = Object.keys(CAT_KEYS).filter(c => !usedCats.includes(c));
  const hasAllTaken = db.contributors.some(c => c.category === 'All' && !c.taskApprovalOnly);
  const allOpt = hasAllTaken ? '' : `<option value="All">All Categories</option>`;
  const catOptions = availCats.map(c => `<option value="${c}">${c}</option>`).join('');
...
function openEditContrib(id) {
  const c = db.contributors.find(x => x.id === id); if (!c) return;
  const usedCats  = db.contributors.filter(x => x.id !== id && x.category !== 'All' && !x.taskApprovalOnly).map(x => x.category);
  const availCats = Object.keys(CAT_KEYS).filter(cat => !usedCats.includes(cat));
  const otherHasAll = db.contributors.some(x => x.id !== id && x.category === 'All' && !x.taskApprovalOnly);
  const allOpt = otherHasAll ? '' : `<option value="All" ${c.category==='All'?'selected':''}>All Categories</option>`;
  const catOptions = availCats.map(cat => `<option value="${cat}" ${c.category===cat?'selected':''}>${cat}</option>`).join('');
...
async function updateContrib(id) {
  ...
  const idx = db.contributors.findIndex(c => c.id === id); if (idx < 0) return;
...
function exportData() {
  ...
  const internId = parseInt(sel.value); const intern = INTERNS.find(i => i.id === internId); if (!intern) return;
  const s = db.submissions[internId] || {}; const ob = getObedience(internId);
  const adminTasks = db.tasks.filter(t => t.assignedTo === internId && !t.isPersonal);
  ...
  const rows = [
    ...
    row('Metric','Value'), row('Total Points',s.points||0), row('College Attendance',s.college||0),
    ...
    row('Total Admin Tasks',adminTasks.length), row('Completed',adminTasks.filter(t=>isTaskDone(t.id,internId)).length),
    row('Report Date',today()), row(''),
    row('Task Title','Category','Points','Due Date','Repeat','Status'),
    ...adminTasks.map(t => row(t.title, t.category, t.points, t.dueDate||'—', t.repeat?`Every ${t.repeat.interval} ${t.repeat.freq}`:'None', isTaskDone(t.id,internId)?'Completed':'Pending'))
  ];
...
async function doReset() {
  INTERNS.filter(i => i.id !== 99).forEach(i => {
    db.submissions[i.id]  = {college:0, office:0, project:0, bugs:0, suggestions:0, dailyCounts:{}, points:0};
...
window.addEventListener('load', async () => {
  ...
  // them logged in — just like WhatsApp, Gmail, and every modern mobile app.
  let restored = false;
  try {
    const raw = localStorage.getItem('txp_session');
    if (raw) {
      const { id } = JSON.parse(raw);
      const user = INTERNS.find(i => i.id === id);
      if (user) {
        // Valid saved session — restore it silently
        ...
        try {
          ...
          // Firebase error — clear broken session and fall through to login
          hideLoading();
          try { localStorage.removeItem('txp_session'); } catch(e2) {}
        }
      }
    }
  } catch(e) { /* malformed localStorage — ignore and show login */ }
