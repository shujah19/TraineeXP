...
// ╔══════════════════════════════════════════════════╗
// ║   SETTINGS SECURITY — DEVELOPER-ONLY CONFIG      ║
// ║   Change this value in source code only.         ║
// ║   Never expose in UI or editable fields.         ║
// ╚══════════════════════════════════════════════════╝
const _S = ["TXP", "@dm", "!n20", "25"]; // split to avoid easy grep — joins to: TXP@dm!n2025
...
let settingsUnlocked = false; // resets on logout
let _settingsFails = 0; // wrong-attempt counter this session
let _settingsCooldown = false; // lockout flag
...
function processRepeatTasks() {
  ...
  for (const tmpl of templates) {
    const r = tmpl.repeat;
    if (!shouldGenerateToday(r, td, tmpl.lastGenerated)) continue;

    // Check if today's instance already exists (prevent duplicates)
    const alreadyExists = db.tasks.some(
      (t) => t.templateId === tmpl.id && t.instanceDate === td,
    );
    ...
    const inst = {
      ...
      templateId: tmpl.id,
      instanceDate: td,
      repeat: null, // instances don't repeat themselves
    };
    db.tasks.push(inst);

    // Notify intern (admin tasks only)
    if (!tmpl.isPersonal) {
    ...
    tmpl.lastGenerated = td;
    changed = true;
  }
  return changed;
}

function shouldGenerateToday(r, td, lastGenerated) {
  if (lastGenerated === td) return false; // already ran today
  ...
  if (freq === "Days") {
    ...
    const daysDiff = Math.floor(
      (new Date(td) - new Date(lastGenerated)) / 86400000,
    );
  ...
  if (freq === "Weeks") {
    ...
    const weeksDiff = Math.floor(
      (new Date(td) - new Date(lastGenerated)) / (7 * 86400000),
    );
    return weeksDiff >= interval || r.days.includes(dow);
  }
  if (freq === "Months") {
    if (!lastGenerated) return true;
...
function verifySettingsCode() {
  ...
  const correct = _S.join(""); // assembles TXP@dm!n2025
  if (entered === correct) {
    ...
    openSettingsLock(); // re-render with error
  }
}
...
function openSendRequest() {
  ...
  // Only show tasks that: require approval, are not done, and don't already have a pending request
  ...
  const approvalTasks = db.tasks.filter(
    (t) =>
      ...
      !t.isPersonal &&
      !t.isTemplate &&
      t.approvalRequired === true && // ← ONLY approval-required tasks
      !isTaskDone(t.id, currentUser.id) &&
      !alreadyRequested.has(t.id), // ← skip if request already pending
  );
  openModal(`<div class="modal-title">📨 Send Request to Admin</div>
    ...
    <div class="form-group">
      ...
      <select id="reqType" onchange="toggleReqType()">
        <option value="entry">Add Entry (attendance/project/etc.)</option>
        <option value="task">Task Completion</option>
      </select>
    </div>
    <div id="reqEntrySection">
      <div class="form-group">
        <label>Category</label>
        <select id="reqCat">
    ...
    <div id="reqTaskSection" style="display:none;">
      <div class="form-group">
        ...
        ${
          approvalTasks.length === 0
            ...
            : `<select id="reqTask">
               ${approvalTasks.map((t) => `<option value="${t.id}">${t.title}</option>`).join("")}
             </select>`
        }
      </div>
    </div>
    <div class="form-group">
      ...
      <textarea id="reqNote" rows="2" placeholder="e.g. Contributor was unavailable today"></textarea>
    </div>
    <div class="modal-actions">
      <button class="btn btn-secondary" onclick="closeModal()">Cancel</button>
...
async function approveRequest(reqId) {
  ...
  if (req.type === "entry") {
    ...
    if (!db.taskCompletions[key]?.done) {
      ...
      db.notifications[iid].unshift({
        id: "n-" + Date.now(),
        msg: `✅ Your task completion request for "${req.taskTitle}" was approved!`,
        type: "request",
        read: false,
        date: today(),
      });
  ...
  // Mark the linked adminNotification as read
  ...
  const ribbon = unread.length
    ? (() => {
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;">
        ...
        <button class="notif-ribbon-dismiss" onclick="markNotifsRead(${id})">✕ Dismiss all</button>
      </div>
      <div style="display:flex;flex-direction:column;gap:3px;">
        ${items.map((n) => `<div style="font-size:12px;padding:4px 8px;background:rgba(255,255,255,.15);border-radius:5px;">• ${n.msg}</div>`).join("")}
...
function renderKanbanCard(task, id) {
  ...
  // Delete button for personal tasks only
  const actions =
...
function openTaskDetail(tid) {
  ...
  if (currentRole === "intern") {
    if (task.isPersonal && task.createdBy == currentUser.id)
      internActions = `<button class="btn btn-danger" onclick="closeModal();deletePersonalTask('${task.id}')">🗑 Delete Task</button>`;
    else if (!done && !task.isPersonal)
      internActions = `<button class="btn btn-secondary" onclick="closeModal();openSendRequest()">📨 Request Approval</button>`;
  }
  openModal(`<div class="modal-title">${task.title}</div>
    ...
    <div style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:12px;">
      ...
      ${task.templateId ? `<span class="badge badge-gray">🔁 Recurring</span>` : task.repeat ? `<span class="badge badge-gray">🔁 Every ${task.repeat.interval} ${task.repeat.freq}</span>` : ""}
        ...
        : `<div class="section-title" style="margin-bottom:8px;">Your Submitted Complaints</div>
         ${mine
           .map((c) => {
             ...
             <div style="font-size:11px;color:var(--text2);margin-top:5px;">${c.date}</div>
           </div>`;
           })
           .join("")}`
    }
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;flex-wrap:wrap;gap:8px;">
      <div class="admin-filter-row">
        ...
        <div style="display:flex;gap:4px;">
          ${["All", "Pending", "Completed"].map((s) => `<button class="task-filter-btn${fs === s ? " active" : ""}" onclick="setATaskFilter('s','${s}')">${s}</button>`).join("")}
        </div>
      </div>
        ...
        <tbody>
          ...
          ${filtered
            .map((t) => {
              ...
              <td style="font-weight:600;">${t.title}${t.isTemplate ? ' <span class="badge badge-gray" style="font-size:9px;">🔁 Template</span>' : t.templateId ? ' <span class="badge badge-gray" style="font-size:9px;">🔁 Instance</span>' : ""}</td>
...
function openAdminTaskDetail(tid) {
  ...
  openModal(`<div class="modal-title">${task.title}</div>
    ...
    <div style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:8px;">
      ...
      ${task.points ? `<span class="badge badge-orange">+${task.points} pts</span>` : ""}
      ${task.dueDate ? `<span class="badge badge-blue">Due: ${task.dueDate}</span>` : ""}
      ${task.isTemplate ? `<span class="badge badge-gray">🔁 Repeating Template</span>` : task.templateId ? `<span class="badge badge-gray">🔁 Recurring Instance</span>` : ""}
      ${task.approvalRequired ? `<span class="badge badge-green">Approval Required</span>` : ""}
      ${done ? `<span class="badge badge-green">✓ Completed</span>` : `<span class="badge badge-orange">Pending</span>`}
    </div>
    <div style="font-size:12px;color:var(--text2);margin-bottom:4px;">Assigned to: <strong>${intern?.name || "Unknown"}</strong></div>
    ${task.isTemplate ? `<div style="font-size:11px;color:var(--text3);">Last auto-generated: ${task.lastGenerated || "Never"}</div>` : ""}
    <div class="modal-actions">
      <button class="btn btn-secondary" onclick="closeModal()">Close</button>
      ${!task.templateId ? `<button class="btn btn-primary" onclick="closeModal();openEditTask('${task.id}')">✏️ Edit</button>` : ""}
    </div>`);
}
...
function openEditTask(tid) {
  ...
  const interns = INTERNS.filter((i) => i.id !== 99);
  const repLabel = _repeatConfig
    ? `Every ${_repeatConfig.interval} ${_repeatConfig.freq}`
    : "None";
  openModal(`<div class="modal-title">✏️ Edit Task</div>
    <div class="form-group"><label>Title *</label><input type="text" id="etTitle" value="${task.title.replace(/"/g, "&quot;")}"></div>
    <div class="form-group"><label>Description</label><textarea id="etDesc" rows="2">${task.description || ""}</textarea></div>
    <div class="form-group"><label>Assign To</label><select id="etAssign">${interns.map((i) => `<option value="${i.id}" ${i.id === task.assignedTo ? "selected" : ""}>${i.name}</option>`).join("")}</select></div>
    ...
    ${
      ...
      🔁 This is a repeating template. Repeat schedule: <strong>${repLabel}</strong>. Future instances will use the updated settings.
    </div>`
        : ""
    }
    ...
    <div id="approvalContribDisplay" style="display:${task.approvalContribId ? "block" : "none"};background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;padding:9px 13px;margin-bottom:9px;font-size:13px;">
      <span id="approvalContribLabel" style="color:#1d4ed8;font-weight:600;">${task.approvalContribId ? "✓ Contributor: " + (db.contributors.find((c) => c.id === task.approvalContribId)?.name || "Unknown") : ""}</span>
    </div>
    <div class="modal-actions">
      <button class="btn btn-danger btn-sm" onclick="closeModal();confirmDeleteAdminTask('${task.id}')">🗑 Delete</button>
      <button class="btn btn-secondary" onclick="closeModal()">Cancel</button>
...
async function saveEditTask(tid) {
  ...
  const newAssignedTo = parseInt(document.getElementById("etAssign").value);
  task.title = title;
  ...
  // If reassigned, notify new intern
...
async function doDeleteAdminTask(tid) {
  ...
  // Clean up completions for deleted task
...
function renderAdminNotifs(ca) {
    ...
    ${requests
      .map((r) => {
        ...
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:6px;">
          ...
          <div style="flex:1;">
            <div style="font-weight:700;font-size:13px;">📨 ${intern?.name || "Unknown"} — ${typeLabel}</div>
            ${r.note ? `<div style="font-size:12px;color:var(--text2);margin-top:2px;">"${r.note}"</div>` : ""}
          </div>
          <div style="font-size:11px;color:var(--text2);">${r.date}</div>
        </div>
  ...
  ca.innerHTML = `<div>
    ...
    ${taskNotifs
      ...
      .join("")}
    ${taskNotifs.length > 0 ? `<div style="margin-top:10px;"><button class="btn btn-danger btn-sm" onclick="clearAdminNotifs()">Clear Task Alerts</button></div>` : ""}
  </div>`;
}
  ...
  ca.innerHTML = `<div>
    ...
    ${visible
      .map((c) => {
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:5px;">
          <div style="font-weight:700;">${c.done ? "<s>" : ""} ${c.subject} ${c.done ? "</s>" : ""}</div>
          <div style="display:flex;gap:5px;align-items:center;">
