...
async function doLogin() {
  ...
  try {
    ...
    const sa = document.getElementById("sbAvatar");
    if (sa) sa.textContent = user.name[0];
    const sn = document.getElementById("sbName");
    if (sn) sn.textContent = user.name;
    const sr = document.getElementById("sbRole");
    if (sr)
      sr.textContent = currentRole === "admin" ? "Administrator" : "Intern";
...
function renderSidebar() {
  ...
  // Desktop sidebar + mobile drawer (logout only visible on mobile via CSS)
  document.getElementById("sidebar").innerHTML =
    '<div class="sidebar-section">Menu</div>' +
    `<div class="sidebar-user-card">
      <div class="sidebar-user-avatar" id="sbAvatar">?</div>
      <div>
        <div class="sidebar-user-name" id="sbName">-</div>
        <div class="sidebar-user-role" id="sbRole">-</div>
      </div>
    </div>` +
    nav
      .map(
