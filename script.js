...
async function saveTask() {
  ...
  const newTask = {
    ...
    createdDate: today(),
    createdAt: Date.now(),
    isTemplate: hasRepeat, // repeat tasks are templates; instances are generated by engine
    lastGenerated: hasRepeat ? null : undefined,
  };
  db.tasks.unshift(newTask); // unshift = add to TOP (newest first in array too)
...
function delContrib(id) {
  openModal(`<div class="modal-title" style="color:#e53e3e;">Delete Contributor?</div>
    ...
    <div class="modal-actions">
      <button class="btn btn-secondary" onclick="closeModal()">Cancel</button>
      <button class="btn btn-danger" onclick="delContrib('${id}')">Delete</button>
    </div>`);
}
...
async function doReset() {
  ...
  db.complaints = [];
  db.adminNotifications = [];
  db.tasks = []; // wipe ALL tasks — admin + personal
  closeModal();
