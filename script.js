      ...
      function updateBadges() {
        ...
        if (currentRole === "intern") {
          ...
          const na = (db.adminNotifications || []).filter(
            (n) => !n.read,
          ).length;
      ...
      function getObedience(id) {
        const tasks = db.tasks.filter(
          (t) => t.assignedTo === id && !t.isPersonal,
        );
        if (!tasks.length) return 0;
        return Math.round(
          (tasks.filter((t) => db.taskCompletions?.[t.id + "-" + id]?.done)
            .length /
            tasks.length) *
            100,
        );
    ...
    <div class="modal-actions">
      ...
      function refreshContribFromCode() {
        ...
        if (contrib)
          document.getElementById("catContribName").textContent =
            contrib.name + " ✓";
    ...
    <div class="kanban-tasks">
      ...
      function renderKanbanCard(task, id) {
        ...
        const pts = task.points
          ? `<span class="kc-pts">⭐${task.points}</span>`
          : "";
    ...
    <div class="modal-actions">
      ...
      async function submitApproval(tid, iid) {
        ...
        if (task.approvalContribId && contrib.id !== task.approvalContribId) {
          const ac = db.contributors.find(
            (c) => c.id === task.approvalContribId,
          );
          showToast(
            "Wrong contributor. Required: " +
              (ac?.name || "assigned contributor"),
            "error",
          );
          return;
        }
    ...
    <div class="modal-actions">
      ...
      function renderInternNotifs(ca) {
        ...
        (db.notifications[currentUser.id] || []).forEach(
          (n) => (n.read = true),
        );
        saveDB();
        updateBadges();
    ...
    <div style="padding:18px;text-align:center;color:var(--text2);background:#fff;border:1px solid var(--border);border-radius:10px;font-size:13px;">🔒 Complaints are private and seen only by Admin.</div>
      ...
      function openComplaintModal() {
        const interns = INTERNS.filter(
          (i) => i.id !== 99 && i.id !== currentUser.id,
        );
    ...
    ${renderAdminTaskOverview(sid)}
      ...
      function renderAdminTaskOverview(sid) {
        ...
        const tasks = db.tasks.filter(
          (t) => t.assignedTo === sid && !t.isPersonal,
        );
        const filtered =
          filter === "Completed"
    ...
    <div class="modal-actions">
      ...
      function handleApprovalToggle(cb) {
        if (cb.checked) openApprovalContribDlg();
          ...
          _approvalContribId = null;
          document.getElementById("approvalContribDisplay").style.display =
            "none";
        }
      }
